...
export class AIAReader {
  ...
  static async parse(fileOrUrl: Blob | string): Promise<AiaReader> {
    ...
    const projectProperties = await readProjectProperties(
    ...
    for (const extension of await AIAReader.generateExtensions(
    ...
    for (const screen of await AIAReader.generateScreens(
      entries.filter(
    ...
    for (const asset of await AIAReader.generateAssets(
      entries.filter(
        (x) =>
