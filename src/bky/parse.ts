...
export function parseBky(bky: string): BlockAst {
  if (bky.trim() === "") {
    throw new Error("Empty BKY input");
  }
  ...
  const root =
    doc.getElementsByTagName("xml")[0] ?? doc.getElementsByTagName("XML")[0];
  if (!root) {
  ...
  for (let i = 0; i < el.childNodes.length; i++) {
    ...
    switch (child.nodeName) {
      case "field":
        ...
        break;
      case "title": {
        const titleName = child.getAttribute("name");
        if (titleName) node.fields[titleName] = child.textContent ?? "";
        break;
      }
      case "value": {
        const innerBlock = firstBlockChild(child);
        if (innerBlock)
          node.values[child.getAttribute("name") ?? ""] =
      ...
      case "statement": {
        const innerBlock = firstBlockChild(child);
        if (innerBlock)
          node.statements[child.getAttribute("name") ?? ""] =
            parseBlock(innerBlock);
        break;
      }
      case "mutation": {
